<template>
  <div class="min-h-screen flex flex-col bg-gray-50 -mt-16 md:mt-0">
    ...
    <div class="flex flex-col flex-1 px-2 sm:px-4 md:px-6 pb-20 -pt-8 md:pt-0 md:pb-4">
      ...
      <div class="bg-white shadow-md rounded-2xl p-3 sm:p-4 md:p-6 lg:p-8 mb-4 sm:mb-6">
      ...
      <div class="border-b mb-4 sm:mb-6 md:mb-8 overflow-x-auto">
        <ul class="flex justify-start space-x-2 sm:space-x-6 md:space-x-8 lg:space-x-12 pb-3 sm:pb-4 min-w-max">
      ...
      <!-- Main Content -->
      <div>
        <!-- Account and Security Tab Content -->
          ...
          <div class="flex flex-col lg:flex-row gap-4 sm:gap-6 md:gap-8">
            ...
            <div class="flex-1 bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div class="p-4 sm:p-6">
                ...
                <form @submit.prevent="changePassword" class="space-y-3 sm:space-y-4 md:space-y-5">
                  <div>
          ...
          <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div class="p-4 sm:p-6">
              ...
              <button 
                ...
                class="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors mb-3 sm:mb-4 md:mb-6 text-xs sm:text-sm disabled:opacity-50 flex items-center justify-center space-x-2"
              >
          ...
          <div class="p-3 sm:p-4 md:p-6">
            ...
            <div class="mt-4 sm:mt-6 md:mt-8 flex justify-end">
              <button
                ...
                class="px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors mb-3 sm:mb-4 md:mb-6 text-xs sm:text-sm"
  ...
  <div v-if="showDeleteConfirmation" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click="preventModalClose">
    <div class="bg-white rounded-xl max-w-md w-full p-4 sm:p-6" @click="preventModalClose">
      <div class="text-center mb-4 sm:mb-6">
...
// Computed property for profile picture with fallback chain
const currentProfilePicture = computed(() => {
  ...
  console.log('Settings - Profile photo sources:', {
    profileStorePhoto: profilePhoto,
    authStorePhoto: authPhoto,
    selectedPhoto: profilePhoto || authPhoto || defaultPhoto
  });
...
const getDeviceInfo = () => {
  ...
  // Detect browser with comprehensive detection logic (same as AccountSettings.vue)
  let browser = 'Unknown'
...
const validateSessionActivity = async (device) => {
  if (!device.sessionId || !authStore.user?.uid) return false
  
  try {
    // Check if the session is still valid by querying the database
    const q = query(
      collection(db, 'deviceSessions'),
      where('userId', '==', authStore.user.uid),
    ...
    const lastActive = sessionData.lastActive?.toDate() || new Date(0)
    const now = new Date()
    const timeSinceLastActive = now - lastActive
    
    // Consider session inactive if no activity for more than 24 hours
...
const handleImageUpload = (event) => {
  ...
  if (file) {
    ...
    console.log('Profile picture selected, ready to save')
  }
}
...
const resetImage = () => {
  ...
  console.log('Profile picture reset, file input cleared')
}

const saveProfilePicture = async () => {
  if (!selectedProfilePicture.value || !authStore.user?.userId) {
    errorMessage.value = 'No image selected or user not authenticated'
    showErrorModal.value = true
    preventBodyScroll()
    return
  }
  ...
  try {
    ...
    if (success) {
      ...
      selectedProfilePicture.value = null
      photoChanged.value = false
      profileImage.value = null // Clear preview since it's now saved
    ...
    errorMessage.value = error.message || 'Failed to save profile picture. Please try again.'
    showErrorModal.value = true
    preventBodyScroll()
  } finally {
...
// Watch for profile changes to update the profile picture
watch(() => profileStore.profile, (newProfile) => {
  console.log('Settings - Profile updated:', newProfile?.photoURL);
}, { deep: true });
...
onMounted(async () => {
  ...
  if (authStore.user?.uid) {
    ...
    // Load profile data to get the current profile picture
    try {
      if (authStore.user?.userId) {
        await profileStore.fetchUserProfile(authStore.user.userId);
        console.log('Settings - Profile loaded:', profileStore.profile?.photoURL);
      }
    } catch (error) {
